...
class LanguageService(private val plugin: Slimefun) {
    ...
    private val obsoleteDefaults = mapOf(
        "actionbar.healing-active" to setOf("<green>治愈之箭 <tier> <gray>恢复中 <white><time>"),
        "actionbar.healing-stored" to setOf("<green>治愈之箭 <tier> <gray>已储存 <white><time>"),
        "items.healing_arrow_i.lore2" to setOf("<gray>每分钟恢复 1 颗心生命上限，共 5 次"),
        "messages.healing-started" to setOf(
            "<green>已使用治愈之箭 <tier>，将在 <time> 内恢复生命上限",
            "<green>已储存治愈之箭 <tier>，总剩余 <time>"
        )
    )

    private val defaults = mapOf(
        "hud.section-title" to "<red><bold>重生</bold></red>",
        ...
        "actionbar.healing-active" to "<green>治愈 I <white><i_time> <gray>| <aqua>治愈 II <white><ii_time> <gray>恢复中",
        "actionbar.healing-stored" to "<green>治愈 I <white><i_time> <gray>| <aqua>治愈 II <white><ii_time> <gray>已储存",
        ...
        "items.healing_core.name" to "<light_purple><i:false>治愈核心",
        ...
        "items.healing_arrow_ii.lore2" to "<gray>每 30 秒恢复 1 颗心生命上限，共 5 次",
        "items.healing_arrow_ii.lore3" to "<gray>生命上限已满时暂停并储存剩余时间",
        "healing.tier-name-i" to "<green>I</green>",
        "healing.tier-name-ii" to "<aqua>II</aqua>",
        "messages.totem-added" to "<green>复活次数 +1（<tier>：<count>/<max>）",
        "messages.totem-full" to "<red>该等级的复活次数已达上限（<max>）",
        "messages.first-join-gift" to "<gold>首次加入赠礼：终极复活次数 +<amount>（当前 <count>）",
        ...
        "messages.healing-started" to "<green>已储存治愈之箭 <tier>：I <i_time>，II <ii_time>",
        "messages.healing-restored" to "<green>治愈生效，生命上限恢复 <hearts> 颗心",
        "messages.healing-complete" to "<green>治愈之箭效果已耗尽",
        "messages.economy-failed" to "<red>金币扣除失败（经济插件返回失败），请检查经济配置",
        "messages.cannot-place" to "<red>这个物品不能放置",
        "messages.cannot-shoot" to "<red>治愈物品只能按说明右键使用，不能作为箭矢射出",
        ...
        "messages.usage" to "<gray>用法：<white>/epochrebirth [menu|get|set|give|resethealth|reload]"
    )
    ...
    fun reload() {
        ...
        yaml.getKeys(true).forEach { key ->
            if (!yaml.isConfigurationSection(key)) {
                yaml.getString(key)?.let { value ->
                    if (value !in obsoleteDefaults[key].orEmpty()) {
                        overrides[key] = value
                    }
                }
            }
